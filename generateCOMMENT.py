...
def _eval_epoch(sess, epoch, mode):
    api,cfg, diff, references, hypotheses = [],[], [], [], []
    ...
    with open('./output/refs_wash.txt', 'a') as f1, open('./output/hyps_wash.txt', 'a') as f2,\
            open('./output/srcs_wash.txt', 'a') as f3, open('./output/cfgs_wash.txt', 'a') as f4, \
            open('./output/apis_wash.txt', 'a') as f5:
        while True:
#             print("Temp",temp)
            ...
            try:
                ...
                api.extend(d.tolist() for d in apis)
        ...
        api = utils.list_strip_eos(api, eos_token_id)
        ...
        for example in range(len(references)):
            ...
            awords = tokenizer.convert_ids_to_tokens(api[example])
            ...
            awords = tx.utils.str_join(awords).replace(" ##", "").replace("[CLS]", "")
